...
namespace Microsoft.Azure.WebJobs.Script.WebHost
{
    public class BlobStorageTriggerMetricsRepository : ITriggerMetricsRepository
    {
        ...
        public async Task SampleAsync(IEnumerable<ITriggerScaleMonitor> monitors)
        {
            // TODO: it's a little strange that this repository method is responsible for taking the next
            // sample for each monitor and persisting it.
            // Instead, I think this method should take a Dictionary<ITriggerScaleMonitor, TriggerMetrics> map - the
            // caller is then responsible for taking the next snapshot, and we just persist here.
            ...
            // When writing the metrics blob back after adding our sample, we'll be filtering out any metrics for functions that
            ...
            foreach (var monitor in monitors)
            {
                ...
                // take the sample and append
                var currSample = await monitor.GetMetricsAsync();
                currMetrics.Add(JObject.FromObject(currSample));
            ...
            // TODO: log the metrics json to provide visibility into trigger activity?
        }
